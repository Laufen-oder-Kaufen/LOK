...
public class Battle {
  ...
  */
    ...
    public static /*ArrayList<com.example.lok.Hero>*/ void getHeroes () throws ClassNotFoundException, SQLException {

    }

    public static void prepare3(ArrayList<Hero> activeHeroList) {
System.out.println("Prepare Battle...");

        ArrayList<Hero> heroList = activeHeroList;
        ArrayList<Monster> enemyList = new ArrayList<Monster>();

        Monster Bat = new Monster("Bat", 101, "W", "C", 20, 15, 0, 5, 5, 10, 5, 20, 20);
        enemyList.add(Bat);
        enemyList.add(Bat);
        enemyList.add(Bat);
    ...
    public static void fight(ArrayList<Hero> heroes, ArrayList<Monster> enemies, LinkedList<Actor> battleSequence) {
        ...
        while ((heroes.size() > 0)) {
            for (int i = 0; i < battleSequence.size(); i++) {
                ...
                System.out.println(actor.getName() +" greift " + victim.getName()+ " an. Schaden: " + damage);
                if (victim.getCurrHP() <= 0) {
                    ...
                    System.out.println("Es stirbt: " + victim.getName());
